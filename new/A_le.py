...
'''
# settings
minimum_evaluation_items    = 20 # annotate this many randomly sampled items first for evaluation data before creating training data
minimum_validation_items    = 10  # annotate this many randomly sampled items first for validation data before creating training data
minimum_training_items      = 5    # minimum number of training items before we first train a model


epochs              = 20 # default number of epochs per training session
#select_per_epoch    = 200  # number to select per epoch per label
select_per_epoch    = 10  # number to select per epoch per label
...
number_least_confidence     = 2
number_margin_confidence    = 3
number_ratio_confidence     = 1
...
if validation_count <  minimum_validation_items:
    ...
    if number_least_confidence + number_margin_confidence + number_ratio_confidence + number_entropy_based > 0:   
        ...
        if number_least_confidence > 0:
            ...
            print(uncert_sampling.least_confidence)
...
if training_count > minimum_training_items:
    ...
    # Dataset generation
    data_dict = {'fscore':accuracies[0], 'auc':accuracies[1]}
